...
// ───────────────────────────────────────────────────────────
// Original circuit ① – “body” SHA-256 checker
// ───────────────────────────────────────────────────────────

/**
 * @title Sha256BodyHasher
 * @notice Proves that SHA-256(body) == bodyHash.
 *
 * @param maxBodyLen fixed length (bytes) of the padded body array.
 *        - `body`       private input (length = maxBodyLen)
 *        - `bodyHash`   public  input (2 field elements ≙ 256 bits)
 */
template Sha256BodyHasher(maxBodyLen) {
    // === INPUTS ===
    // The pre-image, known only to the prover.
    signal input body[maxBodyLen];
    // The expected hash digest, known by everyone.
    signal input bodyHash[2];

    // ---
    // STAGE 1: Convert the body from bytes to bits.
    // ---
    component bytes2bits = Bytes2Bits(maxBodyLen);
    bytes2bits.in <== body;

    component hasher = Sha256(maxBodyLen * 8);
    hasher.in <== bytes2bits.out;

    component n2b_high = Num2Bits(128);
    component n2b_low = Num2Bits(128);
    n2b_high.in <== bodyHash[0];
    n2b_low.in <== bodyHash[1];
...
// ───────────────────────────────────────────────────────────
// Original circuit ② – header parsing + two SHA-256 checks
// ───────────────────────────────────────────────────────────

/**
 * @title ExtractAndVerifyHash
 * @notice Extracts the address inside “from:<...>” and checks that:
 *         1) SHA-256(extracted)   == gmailHash  and
 *         2) SHA-256(full header) == headerHash
 *
 * @param maxSliceLen  length (bytes) of the header buffer
 * @param maxOutputLen maximum bytes to copy out of the “<addr>”
 */
template ExtractAndVerifyHash(maxSliceLen, maxOutputLen) {
    ...
    component bytes2bits2 = Bytes2Bits(maxSliceLen);
    bytes2bits2.in <== header;

    component hasher2 = Sha256(maxSliceLen * 8);
    hasher2.in <== bytes2bits2.out;

    component n2b_high2 = Num2Bits(128);
    component n2b_low2 = Num2Bits(128);
    n2b_high2.in <== headerHash[0];
    n2b_low2.in <== headerHash[1];

    for (var i = 0; i < 128; i++) {
        hasher2.out[i] === n2b_high2.out[127 - i];
        hasher2.out[128 + i] === n2b_low2.out[127 - i];
    }
}


// ───────────────────────────────────────────────────────────
// NEW: wrapper that runs **both** sub-circuits
// ───────────────────────────────────────────────────────────

/**
 * @title CombinedProof
 * @notice One template that proves both:
 *         - SHA-256(body)             == bodyHash
 *         - header   checks & hashes  == gmailHash & headerHash
 *
 * @param maxBodyLen   compile-time constant (bytes)
 * @param maxSliceLen  compile-time constant (bytes)
 * @param maxOutputLen compile-time constant (bytes)
 */
template CombinedProof(maxBodyLen, maxSliceLen, maxOutputLen) {
    // ── public / private inputs ──────────────────────────
    signal input body       [maxBodyLen];   // private
    signal input bodyHash   [2];            // public

    signal input header     [maxSliceLen];  // private
    signal input gmailHash  [2];            // public
    signal input headerHash [2];            // public

    // ── sub-component ① — body hasher ───────────────────
    component bodyH = Sha256BodyHasher(maxBodyLen);
    for (var i = 0; i < maxBodyLen; i++)   bodyH.body[i]     <== body[i];
    for (var i = 0; i < 2;          i++)   bodyH.bodyHash[i] <== bodyHash[i];

    // ── sub-component ② — header extractor + hashes ─────
    component hdr = ExtractAndVerifyHash(maxSliceLen, maxOutputLen);
    for (var i = 0; i < maxSliceLen; i++)  hdr.header[i]      <== header[i];
    for (var i = 0; i < 2;          i++) {
        hdr.gmailHash [i] <== gmailHash [i];
        hdr.headerHash[i] <== headerHash[i];
    }

    // optional flag so the circuit has a conventional output
    signal output ok;
    ok <== 1;   // will be satisfiable only if all internal constraints hold
}
